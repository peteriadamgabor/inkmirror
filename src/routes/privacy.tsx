...
const SectionH = (props: { children: string }) => (
  <h2 class="font-serif text-2xl md:text-3xl tracking-tight text-stone-900 dark:text-stone-100 mb-5">
    {props.children}
  </h2>
);

const SubH = (props: { children: string }) => (
  <h3 class="font-sans text-sm uppercase tracking-widest text-violet-600 dark:text-violet-300/80 mt-8 mb-3">
    {props.children}
  </h3>
);
    ...
    <For each={props.items}>
      {(item) => (
        <li class="flex items-start gap-3">
          <span class="text-violet-600 dark:text-violet-500 mt-2 shrink-0 text-[10px]">◆</span>
          <span>{item}</span>
        </li>
      )}
    </For>
...
const NumberedSteps = (props: { items: string[] }) => (
  <ol class="space-y-3 mb-5 list-decimal pl-6 marker:text-violet-500 dark:marker:text-violet-400/70">
    ...
    <For each={props.items}>
      {(item) => (
        <li class="flex items-start gap-3">
          <span class="text-violet-600 dark:text-violet-500 mt-2 shrink-0 text-[10px]">◆</span>
          <div>
            <span class="text-stone-900 dark:text-stone-200 font-medium">{item.name}.</span>{' '}
            <span>{item.body}</span>
          </div>
        </li>
      )}
...
export const PrivacyRoute = () => {
  ...
  return (
    <div class="min-h-screen inkmirror-public-page inkmirror-paper font-sans">
      ...
      <main class="max-w-3xl mx-auto px-6 pt-32 pb-24 font-serif text-stone-700 dark:text-stone-300 leading-relaxed text-[17px]">
        {/* --- Title --- */}
        <h1 class="font-serif text-4xl md:text-5xl tracking-tight text-stone-900 dark:text-stone-100 mb-3">
          {c().pageTitle.split(' — ')[0]}
        </h1>
        <p class="text-xs text-stone-500 font-sans mb-8 inkmirror-smallcaps tracking-widest">
          {c().lastUpdated}
        </p>
        <p class="text-stone-700 dark:text-stone-300 mb-2 text-[18px]">{c().intro}</p>
        ...
        <section>
          ...
          <ol class="space-y-3 list-decimal pl-6 marker:text-violet-500 dark:marker:text-violet-400/70">
            <li class="pl-1">
              <span class="text-stone-900 dark:text-stone-200 font-medium">
        ...
        <section>
          ...
          <p class="text-stone-500 dark:text-stone-400">{c().neverSeeCloser}</p>
        </section>
        ...
        <section>
          ...
          <p class="mb-2 text-stone-500 dark:text-stone-400">{c().syncPairingNote}</p>
          ...
          <p class="text-stone-500 dark:text-stone-400">{c().syncDeleteOfflineNote}</p>
        </section>
        ...
        <section>
          ...
          <p>
            ...
            <a
              ...
              class="text-violet-600 dark:text-violet-300 hover:text-violet-700 dark:hover:text-violet-200 underline underline-offset-4 decoration-violet-500/40 hover:decoration-violet-300"
            >
              {PRIVACY_CONTACT_EMAIL}
            </a>
        ...
        <section>
          ...
          <div class="overflow-x-auto rounded-xl border border-stone-300/50 dark:border-stone-800/80">
            <table class="w-full text-left text-sm font-sans">
              <thead class="bg-stone-200/60 dark:bg-stone-900/60 text-stone-700 dark:text-stone-300 inkmirror-smallcaps tracking-widest text-xs">
                <tr>
              ...
              <tbody class="text-stone-500 dark:text-stone-400">
                <For each={c().subprocessorsRows}>
                  {(row) => (
                    <tr class="border-t border-stone-300/50 dark:border-stone-800/80">
                      <td class="px-4 py-3 text-stone-900 dark:text-stone-200 font-medium align-top">
                        {row.provider}
                      </td>
        ...
        <section>
          ...
          <p class="text-stone-500 dark:text-stone-400">{c().changesCloser}</p>
        </section>
      </main>
    </div>
  );
