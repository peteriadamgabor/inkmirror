 ...
 * - `connect-src 'self'` — both the HF proxy and the GlitchTip crash
 *   tunnel (/glitchtip-tunnel) are same-origin Worker routes, so no
 *   external connect origin is needed. Direct HF requests from localhost
 *   dev happen before this CSP ships (dev bypasses the Worker entirely).
